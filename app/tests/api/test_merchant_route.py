...
class TestMerchantRoutes:
    @pytest.mark.asyncio
    async def test_merchant_barcode_verify_route_exists(self, app: FastAPI) -> None:
        assert (
            app.url_path_for("merchant:barcode:verify")
            == "/api/merchant/barcode/verify"
        )
    ...
    async def test_merchant_scan_transactions_count_route_exists(
        ...
        assert (
            app.url_path_for("merchant:scan_transactions_count")
            == "/api/merchant/scan_transactions_count"
        )
        assert (
            app.url_path_for("merchant:scan-transactions-count")
    ...
    async def test_merchant_scan_transactions_count_route_raises_error_on_invalid(
        ...
        res = await client.post(
            app.url_path_for("merchant:scan-transactions-count"), json={}
        )
        assert res.status_code == HTTP_422_UNPROCESSABLE_ENTITY
