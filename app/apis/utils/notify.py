...
class Notify:
    ...
    def send_email(self, to: List, template: str, variables: Dict[str, Any]):
        if is_test:
            return
        data = {"to": to, "variables": variables}
        requests.post(
            self.prefix + "/v1/mail/datapassport/{}".format(template),
            json.dumps(data),
            headers=self.headers,
        )

    def send_sms(self, to: List, template: str, variables: Dict[str, Any] = {}):
        if is_test:
            return
        return
